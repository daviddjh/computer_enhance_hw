...
namespace d_std {
    ...
    struct d_stack {
        ...
        void alloc(Memory_Arena *arena, u64 nitems);
    ...
    // Allocate memory for array. Initialize capacity.
    template <typename T> 
    void d_stack<T>::alloc(Memory_Arena *arena, u64 nitems){
    ...
    // Reset size of array to 0. Keep memory.
    template <typename T> 
    void d_stack<T>::reset(){
        size = 0;
    }

    // Release memory of array back to arena.
    template <typename T> 
    void d_stack<T>::release(){
    ...
    // Push Value onto end of array
    template <typename T> 
    void d_stack<T>::push_back(T item){
    ...
    // Set item in array
    template <typename T> 
    void d_stack<T>::set(u64 index, T item){
    ...
    template <typename T> 
    T& d_stack<T>::operator[] (u64 index){
    ...
    template <typename T> 
    T d_stack<T>::operator[] (u64 index) const {
